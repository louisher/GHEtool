...
FOLDER = path.dirname(path.realpath(__file__))  # solve problem with importing GHEtool from sub-folders
...
class Borefield:
    ...
    def gfunction(self, timeValue: list, H: float) -> np.ndarray:
        ...
        if not path.isfile(f'{folder}/Data/{name}'):
            folder = '.'
            if not path.isfile(f'{folder}/Data/{name}'):
    ...
    def createCustomDataset(self, customBorefield, nameDatafile: str, nSegments: int = 12,
        ...
        if not path.isfile(f"Data/{name}"):
